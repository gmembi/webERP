...
if (isset($_POST['PostExchangeDifference']) AND is_numeric(filter_number_format($_POST['DoExchangeDifference']))) {
	...
	if (!is_numeric($_POST['BankStatementBalance'])) {
		...
		$DateExc = FormatDateForSQL($_POST['DateExchange']);
		...
		$sql = "SELECT periodno FROM periods WHERE lastdate_in_period='". $DateExc. "'";
	    $PeriodResult = DB_query($sql);
	    $myrow = DB_fetch_row($PeriodResult);
		$PeriodNo = $myrow[0];
		$result = DB_Txn_Begin();
...
//yet to code the journal
		...
		$SQL = "INSERT INTO gltrans (type,
								  ...
								  VALUES (36,
									...
									'" . $PeriodNo . "',
									'" . $_POST['BankAccount'] . "',
									'" . $CurrencyRow['bankaccountname'] . ' ' . _('reconciliation on') . ' ' . $DateExc  . "',
									'" . (-$ExchangeDifference) . "')";
...
echo '<tr><td>' . _('Date From') . ' (' . $_SESSION['DefaultDateFormat'] . ':</td>
	<td><input type="text" name="DateFrom" class="date"  maxlength="10" size="11" /></td>
	</tr>';


echo '<tr><td>' . _('Date To') . ' (' . $_SESSION['DefaultDateFormat'] . ':</td>
		<td><input type="text" name="DateTo" class="date"  maxlength="10" size="11" /></td>
		</tr>';
...
/*Get the balance of the bank account concerned */
	...
	$sql = "SELECT periodno FROM periods WHERE lastdate_in_period='". $DateTo. "'";
	...
	echo '<table class="selection">
			<tr class="striped_row">
				<td colspan="6"><b>' . $CurrencyRow['bankaccountname'] . ' ' . _('Balance as at') . ' ' . ConvertSQLDate($DateTo);
	...
	$SQL = "SELECT amount/exrate AS amt,
				...
				AND banktrans.transdate BETWEEN '" . $DateFrom . "' AND '" . $DateTo . "'
				AND amount < 0
	...
	if (isset($_POST['DoExchangeDifference'])) {
		...
		echo '<tr>
				...
				<td class="number"><input type="text" name="DateExchange" class="date" alt="'.$_SESSION['DefaultDateFormat'].'" maxlength="10" size="11" onChange="isDate(this, this.value, '."'".$_SESSION['DefaultDateFormat']."'".')" value="' . $_POST['DateExchange'] . '" /><td>
			</tr>
			<tr>
