...
include ('includes/DefineCartInvoiceClass.php');
...
/*now populate the line items array with the sales order details records */
		...
		$LineItemsSQL = "SELECT stkcode,
								...
								salesorderdetails.lgroup,
								salesorderdetails.lsection,
								salesorderdetails.lcost,
								salesorderdetails.source,
		...
		if (DB_num_rows($LineItemsResult)>0) {
			...
			while ($myrow=DB_fetch_array($LineItemsResult)) {
				...
				$_SESSION['Items'.$identifier]->add_to_cart($myrow['stkcode'],
											...
											$myrow['lsection'],
											$myrow['lcost'],
											$myrow['source']);	/*NB NO Updates to DB */
...
foreach ($_SESSION['Items'.$identifier]->LineItems as $LnItm) {
	...
	echo '<td>' . $LnItm->StockID . '</td>
		...
		<td class="text">' . $LnItm->LGroup . '</td>
		<td class="text">' . $LnItm->LSection . '</td>
		<td class="text">' . $LnItm->LCost . '</td>
		<td class="text">' . $LnItm->Source . '</td>
...
/*end of process invoice */
	...
	echo '<table class="selection">
		...
		echo '<tr><td>' . _('Select Bank Accounts') . ':</td>
		<td><select tabindex="'.$j.'" name="LBank" onChange="return ReloadForm(UpdateBank)">';
	...
	if (DB_num_rows($result)==0){
		...
		while ($myrow=DB_fetch_array($result)){
			if (isset($_POST['LBank']) AND ($_POST['LBank']==$myrow['accountcode'])){
				echo '<option selected="True" value="' . $myrow['accountcode'] . '">' . $myrow['accountcode'] . ' - ' . $myrow['bankaccountnumber'] . '</option>';
			} else {
				echo '<option value="' . $myrow['accountcode'] . '">' . $myrow['accountcode'] . ' - ' . $myrow['bankaccountnumber'] . '</option>';
			}
		}
		echo '</select></td></tr>';
	}
