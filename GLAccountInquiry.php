...
$result=DB_query($SQL);
echo '<option value="0">0 - '._('All cost') . '</option>';
...
echo '</select></td>
	...
	echo '<option value="0"> 0 - '._('All source') . '</option>';
	while ($myrow=DB_fetch_array($result)){
...
if(isset($_POST['Show'])) {
	...
	if($_POST['LCost']!=0 ) {
 		$sql = $sql . " AND gltrans.loccost='" . $_POST['LCost'] . "'";
	}
	
    if($_POST['source']!=0) {
		$sql = $sql . "  AND gltrans.source='" . $_POST['source'] . "'";
	}
	// if($_POST['LCost']!=0 && $_POST['source']===0) {
	// 	$sql = $sql . "  AND loccost='" . $_POST['LCost'] . "'";
    // }
	...
	while($myrow=DB_fetch_array($TransResult)) {
		...
		echo	'<tr class="striped_row">
				...
				<td class="text">', $myrow['costname'], '</td>
				<td class="text">', $myrow['sourcedescription'], '</td>';
