...
//Select the tag
echo '<tr><td>' . _('Select Cost/Tag') . ':</td><td><select name="cost">';
...
echo '<option value=0>0 - '._('All cost/tags') . '</option>';
...
if (isset($_POST['Show'])){
	...
	while ($myrow=DB_fetch_array($TransResult)) {
		...
		$costsql="SELECT costname FROM locationcost WHERE costid='".$myrow['loccost'] . "'";
		$costresult=DB_query($costsql,$db);
		$costrow = DB_fetch_array($costresult);
		if ($costrow['costname']=='') {
			$costrow['costname']=_('None');
		}
		printf('<td>%s</td>
