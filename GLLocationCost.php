...
$Title = _('Location Cost');
$ViewTopic = 'GeneralLedger';
$BookMark = 'LocationCost';
...
if (isset($_POST['submit'])) {
	...
	$sql="SELECT count(costid)
			FROM locationcost
			WHERE costid='".$_POST['CostID']."'";
...
//	}
	...
	if ($_POST['SelectedLocationCost']!='' AND $InputError !=1) {
		...
		$sql = "UPDATE locationcost SET costid='" . $_POST['CostID'] . "',
										costname='" . $_POST['CostName'] . "',
										
										locsectionname='" . $_POST['SectionName'] . "'
									WHERE costid = '" . $_POST['SelectedLocationCost'] . "'";
	...
	/*Selected group is null cos no item selected on first time round so must be adding a record must be submitting new entries in the new account group form */
		...
		$sql = "INSERT INTO locationcost ( costid,
											costname,
											locsectionname
										) VALUES (
											'" . $_POST['CostID'] . "',
	...
	if ($InputError!=1){
		...
		$result = DB_query($sql,$db,$ErrMsg,$DbgMsg);
		prnMsg($msg,'success');
		unset ($_POST['SelectedLocationCost']);
		unset ($_POST['CostID']);
...
// PREVENT DELETES IF DEPENDENT RECORDS IN 'LocationCost'

	
//		$sql = "SELECT COUNT(sectionname) AS section FROM locationcost WHERE sectionname = '" . $_GET['SelectedLocationSection'] . "'";
...
//		} else {
			$sql="DELETE FROM locationcost WHERE costid='" . $_GET['SelectedLocationCost'] . "'";
			$ErrMsg = _('An error occurred in deleting the account group');
			$DbgMsg = _('The SQL that was used to delete the account group was');
			$result = DB_query($sql,$db,$ErrMsg,$DbgMsg);
			prnMsg( $_GET['SelectedLocationCost'] . ' ' . _('section has been deleted') . '!','success');
//		}
...
or deletion of the records*/
	...
	$sql = "SELECT costid,
					...
					sectionname
			FROM locationcost
			LEFT JOIN locationsection ON sectionid = locsectionname
			ORDER BY sectionid";
    ...
    _('Location Cost'), '" /> ', // Icon title.
    _('Location Cost'), '</p>';// Page title.
...
//		} //end of switch statement
		...
		echo '<td>' . $myrow['costid'] . '</td>
			<td>' . htmlentities($myrow['costname'], ENT_QUOTES,'UTF-8') . '</td>
			<td>' . $myrow['sectionname'] . '</td>';
		echo '<td><a href="' . htmlspecialchars($_SERVER['PHP_SELF'], ENT_QUOTES, 'UTF-8') . '?SelectedLocationCost=' . htmlentities($myrow['costid'], ENT_QUOTES,'UTF-8') . '">' . _('Edit') . '</a></td>';
		echo '<td><a href="' . htmlspecialchars($_SERVER['PHP_SELF'], ENT_QUOTES, 'UTF-8') . '?SelectedLocationCost=' . htmlentities($myrow['costid'], ENT_QUOTES,'UTF-8') . '&amp;delete=1" onclick="return confirm(\'' . _('Are you sure you wish to delete this account group?') . '\');">' . _('Delete') .'</a></td></tr>';
...
if (!isset($_GET['delete'])) {
	...
	echo '<form method="post" id="LocationCost" action="' . htmlspecialchars($_SERVER['PHP_SELF'], ENT_QUOTES, 'UTF-8') . '">';
	echo '<input type="hidden" name="FormID" value="' . $_SESSION['FormID'] . '" />';

	if (isset($_GET['SelectedLocationCost'])) {
		...
		$sql = "SELECT costid,
						...
						locsectionname
				FROM locationcost
				WHERE costid='" . $_GET['SelectedLocationCost'] ."'";
		...
		$_POST['CostID'] = $myrow['costid'];
		$_POST['CostName']  = $myrow['costname'];
		$_POST['SectionName'] = $myrow['locsectionname'];

		echo '<table class="selection">';
		echo '<tr>
				<th colspan="2" class="header">' . _('Edit Location Section Details') . '</th>
			</tr>';
		echo '<input type="hidden" name="SelectedLocationCost" value="' . $_GET['SelectedLocationCost'] . '" />';
		echo '<input type="hidden" name="CostID" value="' . $_POST['CostID'] . '" />';
                 echo '<input type="hidden" name="CostName" value="' . $_POST['CostName'] . '" />';
		...
		if (!isset($_POST['SelectedLocationCost'])){
			$_POST['SelectedLocationCost']='';
		}
		if (!isset($_POST['CostID'])){
			$_POST['CostID']='';
		}
		if (!isset($_POST['CostName'])){
			$_POST['CostName']='';
		}
		...
		echo '<br /><table class="selection">';
		echo '<input  type="hidden" name="SelectedLocationCost" value="' . $_POST['SelectedLocationCost'] . '" />';
		echo '<tr>
				<th colspan="2" class="header">' . _('New Location Section Details') . '</th>
			</tr>';
		echo '<tr>
				<td>' . _('Location Cost ID') . ':' . '</td>
				<td><input tabindex="1" ' . (in_array('CostID',$Errors) ?  'class="inputerror"' : '' ) .' type="text" name="CostID" size="50" maxlength="50" value="' . $_POST['CostID'] . '" /></td>
			</tr>';
                echo '<tr>
				<td>' . _('Location Cost Name') . ':' . '</td>
				<td><input tabindex="1" ' . (in_array('CostName',$Errors) ?  'class="inputerror"' : '' ) .' type="text" name="CostName" size="50" maxlength="50" value="' . $_POST['CostName'] . '" /></td>
			</tr>';
	}
	

	echo '<tr>
			<td>' . _('Location Section') . ':' . '</td>
			<td><select tabindex="3" ' . (in_array('SectionName',$Errors) ?  'class="selecterror"' : '' ) . ' name="SectionName">';

	$sql = "SELECT sectionid, sectionname FROM locationsection ORDER BY sectionid";
	$secresult = DB_query($sql, $db,$ErrMsg,$DbgMsg);
	while( $secrow = DB_fetch_array($secresult) ) {
		if ($_POST['SectionName']==$secrow['sectionid']) {
	...
	if (isset($_POST['SelectedLocationCost']) or isset($_GET['SelectedLocationCost'])) {
		echo '<div style="text-align: right"><a href="' . htmlspecialchars($_SERVER['PHP_SELF'], ENT_QUOTES, 'UTF-8') . '">' . _('Review Account Groups') . '</a></div>';
	}

	echo '<script  type="text/javascript">defaultControl(document.forms[0].CostID);</script>';
