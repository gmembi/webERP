...
$Title = _('Income and Expenditure by Location Cost');
...
if ((!isset($_POST['PeriodFrom']) AND !isset($_POST['PeriodTo'])) OR isset($_POST['NewReport'])) {
	...
	while ($MyRow=DB_fetch_array($result)) {
    	if (isset($_POST['cost']) and $_POST['cost']==$MyRow['costid']) {
			echo '<option selected="selected" value="' . $MyRow['costid'] . '">' . $MyRow['costid'].' - ' .$MyRow['costname'] . '</option>';
    	} else {
...
// End select tag
	...
	$grpsql="SELECT costname FROM locationcost WHERE costid='".$_POST['cost']."'";
	$grpresult=DB_query($grpsql);
	$grprow=DB_fetch_array($grpresult);
	$Grp=$grprow['groupname'];
	...
	$SQL = "SELECT accountgroups.sectioninaccounts,
			...
			AND gltrans.loccost='" . $_POST['cost'] . "'
	...
	include('includes/PDFCostProfitAndLossPageHeader.inc');
...
//			$GrpPrdLY[$i] +=$AccountPeriodLY;
	...
	$SQL="SELECT costname FROM locationcost WHERE costid='".$_POST['cost']."'";
	$result=DB_query($SQL);
	...
	echo '<tr>
			<th colspan="9">
				<div class="centre">
					<h2><b>' . _('Statement of Income and Expenditure for Location Cost'). ' ' . $MyRow[0]. ' ' . _('during the'). ' ' . $NumberOfMonths . ' ' . _('months to'). ' ' . $PeriodToDate . '</b></h2>
				</div>
			</th>
		</tr>';
