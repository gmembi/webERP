...
$Title = _('Income and Expenditure by Location Group');
...
if ((!isset($_POST['PeriodFrom']) AND !isset($_POST['PeriodTo'])) OR isset($_POST['NewReport'])) {
	...
	while ($MyRow=DB_fetch_array($result)) {
    	if (isset($_POST['group']) and $_POST['group']==$MyRow['groupid']) {
...
// End select tag
	...
	$grpsql="SELECT groupname FROM locationgroup WHERE groupid='".$_POST['group']."'";
	...
	$SQL = "SELECT accountgroups.sectioninaccounts,
			...
			AND gltrans.locgroup='" . $_POST['group'] . "'
	...
	include('includes/PDFGroupProfitAndLossPageHeader.inc');
...
//			$GrpPrdLY[$i] +=$AccountPeriodLY;
	...
	$SQL="SELECT groupname FROM locationgroup WHERE groupid='".$_POST['group']."'";
	$result=DB_query($SQL);
	...
	echo '<tr>
			<th colspan="9">
				<div class="centre">
					<h2><b>' . _('Statement of Income and Expenditure for Location Group'). ' ' . $MyRow[0]. ' ' . _('during the'). ' ' . $NumberOfMonths . ' ' . _('months to'). ' ' . $PeriodToDate . '</b></h2>
				</div>
			</th>
		</tr>';
