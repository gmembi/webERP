...
if ((!isset($_POST['PeriodFrom']) AND !isset($_POST['PeriodTo'])) OR isset($_POST['NewReport'])) {
	...
	//Select the tag
	echo '<tr>
			<td>' . _('Select Location Section') . '</td>
	...
	while ($MyRow=DB_fetch_array($result)) {
    	if (isset($_POST['section']) and $_POST['section']==$MyRow['sectionid']) {
			echo '<option selected="selected" value="' . $MyRow['sectionid'] . '">' . $MyRow['sectionid'].' - ' .$MyRow['sectionname'] . '</option>';
    	} else {
...
// End select tag
	...
	$grpsql="SELECT sectionname FROM locationsection WHERE sectionid='".$_POST['section']."'";
	$grpresult=DB_query($grpsql);
	$grprow=DB_fetch_array($grpresult);
	$Grp=$grprow['sectionname'];
	...
	$SQL = "SELECT accountgroups.sectioninaccounts,
			...
			AND gltrans.locsection='" . $_POST['section'] . "'
	...
	include('includes/PDFSectionProfitAndLossPageHeader.inc');
	...
	while ($MyRow = DB_fetch_array($AccountsResult)) {
		...
		if ($MyRow['sectionname'] != $ActGrp) {
			if ($ActGrp != '') {
				if ($MyRow['parentgroupname']!=$ActGrp) {
					while ($MyRow['sectionname']!=$ParentGroups[$Level] AND $Level>0) {
...
// Print heading if at end of page
		...
		if ($MyRow['sectionname'] != $ActGrp) {
			...
			$ActGrp = $MyRow['sectionname'];
			$ParentGroups[$Level]=$ActGrp;
			if ($_POST['Detail'] == 'Detailed') {
				$FontSize =10;
				$pdf->setFont('','B');
				$LeftOvers = $pdf->addTextWrap($Left_Margin,$YPos,200,$FontSize,$MyRow['sectionname']);
				$YPos -= (2 * $line_height);
				$FontSize =8;
				$pdf->setFont('','');
			}
...
//			$GrpPrdLY[$i] +=$AccountPeriodLY;
	...
	$pdf->OutputD($_SESSION['DatabaseName'] . '_' .'Group_Income_Statement_' . date('Y-m-d').'.pdf');
	$pdf->__destruct();
	exit;
	...
	$SQL="SELECT sectionname FROM locationsection WHERE sectionid='".$_POST['section']."'";
	$result=DB_query($SQL);
	...
	echo '<tr>
			<th colspan="9">
				<div class="centre">
					<h2><b>' . _('Statement of Income and Expenditure for Location Section'). ' ' . $MyRow[0]. ' ' . _('during the'). ' ' . $NumberOfMonths . ' ' . _('months to'). ' ' . $PeriodToDate . '</b></h2>
				</div>
			</th>
		</tr>';
	...
	while ($MyRow=DB_fetch_array($AccountsResult)) {
		...
		if ($MyRow['sectionname']!= $ActGrp) {
			...
			$ParentGroups[$Level] = $MyRow['sectionname'];
			$ActGrp = $MyRow['sectionname'];
			if ($_POST['Detail']=='Detailed') {
				printf('<tr>
