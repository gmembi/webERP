...
include('includes/DefinePaymentRequestClass.php');
include('includes/session.php');
$Title = _('Payment Entry');
if(isset($_GET['SupplierID'])) {// Links to Manual before header.php
	$ViewTopic = 'AccountsPayable';
	$BookMark = 'SupplierPaymentsRequest';
	$PageTitleText = _('Supplier Payment Request Entry ');
} else {
	$ViewTopic= 'GeneralLedger';
	$BookMark = 'BankAccountPaymentsRequest';
	$PageTitleText = _('Bank Account Payments Request Entry');
}
	...
	
     {
		...
		$result = DB_query($SQL,$ErrMsg,_('The SQL that failed was'),true);
				...
				foreach($_SESSION['PaymentDetail'.$identifier]->GLItems as $PaymentItem) {
					...
					$SQL = "INSERT INTO payrequestdetails (
								PaymentNo,
                                narrative,
							    locgroup,
							    locsection,
		...
		DB_Txn_Commit();
		prnMsg(_('Payment Request') . ' ' . $TransNo . ' ' . _('has been successfully entered'),'success');
...
// End select source
	...
	echo '<tr>
			<td>' . _('Payee Name') . '</td>
			<td><input type="text" name="PayeeName" value="' . $_POST['PayeeName'] . '" maxlength="50" size="50" /></td>
		</tr>';
		...
		foreach($_SESSION['PaymentDetail'.$identifier]->GLItems as $PaymentItem) {
			...
			if ($PaymentItem->LGroup==0) {
				...
				$GrpName=$GrpMyrow[0];
						...
						FROM locationsection
						WHERE sectionid='" . $PaymentItem->LSection . "'";
			$SecResult=DB_query($secsql, $db);
			$SecMyrow=DB_fetch_row($SecResult);
			if ($PaymentItem->LSection==0) {
				//$SecName='None';
			} else {
				$SecName=$SecMyrow[0];
			}
			...
			echo '<tr>
				...
				<td>' . $PaymentItem->LGroup . ' - ' . $PaymentItem->LGroup . '</td>
                <td>' . $PaymentItem->LSection . ' - ' . $PaymentItem->LSection . '</td> 
                <td>' . $PaymentItem->LCost . ' - ' . $PaymentItem->LCost . '</td>    
				<td>' . $PaymentItem->source . ' - ' . $PaymentItem->source . '</td>  
...
the fields for entry of receipt amt and disc */
	...
	echo '<table class="selection">
			<tr>
				<th colspan="2"><h3>', _('Supplier Payment Request Entry'), '</h3></th>
			</tr>
			</table>';
	...
	<br />
	<div class="centre"><input type="submit" name="CommitBatch"  value="' . _('Accept and Process Payment') . '" /></div>';
}
echo '</div>
	</form>';
