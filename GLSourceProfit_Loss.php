...
if ((!isset($_POST['PeriodFrom']) AND !isset($_POST['PeriodTo'])) OR isset($_POST['NewReport'])) {
	...
	while ($MyRow=DB_fetch_array($result)) {
    	if (isset($_POST['source']) and $_POST['source']==$MyRow['sourceref']) {
			echo '<option selected="selected" value="' . $MyRow['sourceref'] . '">' . $MyRow['sourceref'].' - ' .$MyRow['sourcedescription'] . '</option>';
    	} else {
...
// End select tag
	...
	$sourcesql="SELECT sourcedescription FROM source WHERE sourceref='".$_POST['source']."'";
	$sourceresult=DB_query($sourcesql);
	$sourcerow=DB_fetch_array($sourceresult);
	$Source=$sourcerow['sourcedescription'];
	...
	$SQL = "SELECT accountgroups.sectioninaccounts,
			...
			AND gltrans.source='" . $_POST['source'] . "'
	...
	include('includes/PDFSourceProfitAndLossPageHeader.inc');
...
//			$GrpPrdLY[$i] +=$AccountPeriodLY;
	...
	$SQL="SELECT sourcedescription FROM source WHERE sourceref='".$_POST['source'] . "'";
	$result=DB_query($SQL);
	...
	echo '<tr>
			<th colspan="9">
				<div class="centre">
					<h2><b>' . _('Statement of Income and Expenditure for Source'). ' ' . $MyRow[0]. ' ' . _('during the'). ' ' . $NumberOfMonths . ' ' . _('months to'). ' ' . $PeriodToDate . '</b></h2>
				</div>
			</th>
		</tr>';
