...
/*Now figure out the inventory data to report for the category range under review */
	if ($_POST['Location']=='All'){
		$SQL = "SELECT stockmaster.categoryid,
					...
					SUM(locstock.quantity) *(stockmaster.lastcost) AS lastcosttotal
				FROM stockmaster,
					stockcategory,
					locstock
		...
		$SQL = "SELECT stockmaster.categoryid,
					...
					locstock.quantity *(stockmaster.lastcost) AS lastcosttotal
				FROM stockmaster,
					stockcategory,
					locstock
				INNER JOIN locationusers ON locationusers.loccode=locstock.loccode AND locationusers.userid='" .  $_SESSION['UserID'] . "' AND locationusers.canview=1
...
if (isset($_POST['PrintPDF'])){
	...
	while ($InventoryValn = DB_fetch_array($InventoryResult)){
		...
		if ($_POST['DetailedReport']=='Yes'){
			...
			$LeftOvers = $pdf->addTextWrap($Left_Margin,$YPos,50,$FontSize,$InventoryValn['stockid']);
			$LeftOvers = $pdf->addTextWrap(100,$YPos,220,$FontSize,$InventoryValn['description']);
			$DisplayUnitCost = locale_number_format($InventoryValn['unitcost'],$_SESSION['CompanyRecord']['decimalplaces']);
			$DisplayLastUnitCost = locale_number_format($InventoryValn['lastcost'],$_SESSION['CompanyRecord']['decimalplaces']);
			...
			$DisplayLastItemTotal = locale_number_format($InventoryValn['lastcosttotal'],$_SESSION['CompanyRecord']['decimalplaces']);

			$LeftOvers = $pdf->addTextWrap(250,$YPos,60,$FontSize,$DisplayQtyOnHand,'right');
			$LeftOvers = $pdf->addTextWrap(310,$YPos,15,$FontSize,$InventoryValn['units'],'left');
			$LeftOvers = $pdf->addTextWrap(330,$YPos,60,$FontSize,$DisplayUnitCost, 'right');
			$LeftOvers = $pdf->addTextWrap(390,$YPos,60,$FontSize,$DisplayLastUnitCost, 'right');

			$LeftOvers = $pdf->addTextWrap(450,$YPos,60,$FontSize,$DisplayItemTotal, 'right');
			$LeftOvers = $pdf->addTextWrap(510,$YPos,60,$FontSize,$DisplayLastItemTotal, 'right');
		}
		$Tot_Val += $InventoryValn['itemtotal'];
		$Last_Tot_Val += $InventoryValn['lastcosttotal'];
...
/*Print out the category totals */
	...
	$LeftOvers = $pdf->addTextWrap(450,$YPos,80,$FontSize,$DisplayCatTotVal, 'right');
	$DisplayCatTotQty = locale_number_format($CatTot_Qty,2);
	$LeftOvers = $pdf->addTextWrap(250,$YPos,60,$FontSize,$DisplayCatTotQty, 'right');
...
/*Print out the grand totals */
	...
	$CSVListing = _('Category ID') .','. _('Category Description') .','. _('Stock ID') .','. _('Description') .','. _('Decimal Places') .','. _('Qty On Hand') .','. _('Units') .','. _('Last Cost') .', '. _('Unit Cost') .','. _('Total') .','. _('Last Cost Total') . "\n";
