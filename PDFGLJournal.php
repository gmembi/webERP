...
if ($JournalNo=='Preview') {
	...
	$sql="SELECT gltrans.typeno,
			...
			LEFT JOIN locationcost
				ON gltrans.loccost=locationcost.costid
			LEFT JOIN source
				ON gltrans.source=source.sourceref		
...
while ($counter<=$LineCount) {
	if ($JournalNo=='Preview') {
		...
		$Amount = $myrow['amount'];
		$Cost = $myrow['loccost'].' - '.$myrow['costname'];
		$Source = $myrow['source'].' - '.$myrow['sourcedescription'];
		$JobRef = $myrow['jobref'];
	}
	...
	$LeftOvers = $pdf->addTextWrap($FormDesign->Data->Column5->x,$Page_Height-$YPos,$FormDesign->Data->Column5->Length,$FormDesign->Data->Column5->FontSize, $Cost);
	$LeftOvers = $pdf->addTextWrap($FormDesign->Data->Column6->x,$Page_Height-$YPos,$FormDesign->Data->Column6->Length,$FormDesign->Data->Column6->FontSize, $Source);
	$LeftOvers = $pdf->addTextWrap($FormDesign->Data->Column7->x,$Page_Height-$YPos,$FormDesign->Data->Column7->Length,$FormDesign->Data->Column7->FontSize, $JobRef, 'left');
	$YPos += $line_height;
	$counter++;
