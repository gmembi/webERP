...
if (isset($OrderNo) AND $OrderNo != '' AND $OrderNo > 0 AND $OrderNo != 'Preview') {
	...
	$sql = "SELECT	purchorders.supplierno,
					...
					purchorders.dateauth,
					purchorders.authorisedby,
					purchorders.deladd1,
					purchorders.deladd2,
					purchorders.deladd3,
					purchorders.deladd4,
	...
	$POHeader['authorisedby'] = str_pad('', 50, 'x');
...
if (isset($MakePDFThenDisplayIt) or isset($MakePDFThenEmailIt)) {
	...
	if ($OrderNo == 'Preview' or DB_num_rows($result) > 0) {
        ...
        $pdf->addText($FormDesign->OrderDate->x+70,$Page_Height - $FormDesign->OrderDate->y, $FormDesign->OrderDate->FontSize, ConvertSQLDate($POHeader['orddate']));
       /*Now the Initiator */
$pdf->addText($FormDesign->Initiator->x,$Page_Height - $FormDesign->Initiator->y, $FormDesign->Initiator->FontSize, _('Initiator').': ');
$pdf->addText($FormDesign->Initiator->x+50,$Page_Height - $FormDesign->Initiator->y, $FormDesign->Initiator->FontSize, $POHeader['initiator']);

$pdf->addText($FormDesign->AuthorisedDate->x,$Page_Height - $FormDesign->AuthorisedDate->y, $FormDesign->AuthorisedDate->FontSize, _('Date Authorised') . ':' );
$pdf->addText($FormDesign->AuthorisedDate->x+90,$Page_Height - $FormDesign->AuthorisedDate->y, $FormDesign->AuthorisedDate->FontSize, ConvertSQLDate($POHeader['dateauth']));
/*Now the Initiator */
$pdf->addText($FormDesign->Authoriser->x,$Page_Height - $FormDesign->Authoriser->y, $FormDesign->Authoriser->FontSize, _('Authorised By').': ');
$pdf->addText($FormDesign->Authoriser->x+80,$Page_Height - $FormDesign->Authoriser->y, $FormDesign->Authoriser->FontSize, $POHeader['authorisedby']);

$pdf->addText($FormDesign->PaidDate->x,$Page_Height - $FormDesign->PaidDate->y, $FormDesign->PaidDate->FontSize, _('Date Printed') . ':' );
$pdf->addText($FormDesign->PaidDate->x+80,$Page_Height - $FormDesign->PaidDate->y, $FormDesign->PaidDate->FontSize, Date($_SESSION['DefaultDateFormat']));
/*Now the Initiator */
$pdf->addText($FormDesign->PaidBy->x,$Page_Height - $FormDesign->PaidBy->y, $FormDesign->PaidBy->FontSize, _('Printed By ').': ');
$sql="SELECT realname FROM www_users WHERE userid='".$_SESSION['UserID']."'";
$payerresult=DB_query($sql);
$myrow1=DB_fetch_array($payerresult);
$pdf->addText($FormDesign->PaidBy->x+80,$Page_Height - $FormDesign->PaidBy->y, $FormDesign->PaidBy->FontSize, ($myrow1['realname']));
	...
	if ($ViewingOnly == 0 AND $Success == 1) {
		...
		$sql = "UPDATE purchorders	SET	allowprint =  0,
										...
										status = 'Printed',
										printedby = '" .$_SESSION['UsersRealName']. "',
