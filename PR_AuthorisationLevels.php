...
$Title = _('Payment Request Authorisation Maintenance');
...
if (isset($_POST['Submit'])) {
	...
	if ($myrow[0]==0 and $InputError==0) {
		$sql="INSERT INTO payrequestauth (
			userid,
			currabrev,
			cancreate,
			offhold,
...
if (isset($_GET['Delete'])) {
	$sql="DELETE FROM payrequestauth
...
$sql="SELECT
	payrequestauth.userid,
	www_users.realname,
	currencies.currabrev,
	currencies.currency,
	currencies.decimalplaces,
	payrequestauth.cancreate,
	payrequestauth.offhold,
	payrequestauth.authlevel
	FROM (payrequestauth
	LEFT JOIN www_users ON payrequestauth.userid=www_users.userid)
	LEFT JOIN currencies ON payrequestauth.currabrev=currencies.currabrev";
...
while ($myrow=DB_fetch_array($Result)) {
	...
	echo '<td class="number">'.locale_number_format($myrow['authlevel'],$myrow['decimalplaces']).'</td>';
	echo '<td><a href="'.$RootPath.'/PR_AuthorisationLevels.php?Edit=Yes&UserID=' . $myrow['userid'] . '&Currency='.$myrow['currabrev'].'">'._('Edit').'</td>';
	echo '<td><a href="'.$RootPath.'/PR_AuthorisationLevels.php?Delete=Yes&UserID=' . $myrow['userid'] . '&Currency='.$myrow['currabrev'].'">'._('Delete').'</td></tr>';
}
...
echo '<tr><td>'._('User can create payment voucher').'</td>';
...
echo '<tr><td>'._('User can release voucher').'</td>';
...
echo '<tr><td>'._('User can authorise voucher up to :').'</td>';
if ($Currency!='') {
	echo '<td><input type="input" name="AuthLevel" size="11" class="number" value="'.locale_number_format($AuthLevel, $Currency).'" /></td></tr>';
} else {
