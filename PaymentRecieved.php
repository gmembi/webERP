			...
			if (isset($RequestID)) {
				...
				if (DB_num_rows($Result)==0) {
					$SQL="UPDATE stockrequest
						SET closed=1, fullfill_by = '" . $NameRow['realname'] . "'
					WHERE dispatchid='".$RequestID."'";
					$Result=DB_query($SQL);
				}
			}
...
if (isset($_GET['PRNumber'])) {
	$sql="SELECT stockrequest.dispatchid,
		...
		LEFT JOIN locationgroup
			ON stockrequest.lgroup=locationgroup.groupid
		LEFT JOIN locationsection
			ON stockrequest.lsection=locationsection.sectionid
	...
	while ($myrow=DB_fetch_array($result)) {
		...
		while ($LineRow=DB_fetch_array($LineResult)) {
			echo '<tr>
					...
					<td><select name="'. $LineRow['dispatchid'] . 'Source'. $LineRow['dispatchitemsid'] . '">';
			...
			$SourceResult=DB_query($SQL);
			echo '<option value=0>0 - None</option>';
			while ($mysourcerow=DB_fetch_array($SourceResult)){
				if (isset($_SESSION['Adjustment']->source) and $_SESSION['Adjustment']->source==$mysourcerow['sourceref']){
					echo '<option selected="selected" value="' . $mysourcerow['sourceref'] . '">' . $mysourcerow['sourceref'].' - ' .$myrow['sourcedescription'] . '</option>';
				} else {
					echo '<option value="' . $mysourcerow['sourceref'] . '">' . $mysourcerow['sourceref'].' - ' .$mysourcerow['sourcedescription'] . '</option>';
				}
			}
			echo '</select></td>';
// End select tag
