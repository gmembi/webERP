				...
				if ($Quantity<0 OR $Price <0 OR $DiscountPercentage >100 OR $DiscountPercentage <0){
					...
					if ($WithinCreditLimit AND $Quantity >0){
						$_SESSION['Items'.$identifier]->update_cart_item($OrderLine->LineNumber,
																		...
																		$_POST['LGroup_' . $OrderLine->LineNumber],
																        $_POST['LSection_' . $OrderLine->LineNumber],
                                                                        $_POST['LCost_' . $OrderLine->LineNumber],
																        $_POST['Source_' . $OrderLine->LineNumber],
																		$identifier);
...
/*			if (in_array($_SESSION['PageSecurityArray']['OrderEntryDiscountPricing'], $_SESSION['AllowedPageSecurityTokens'])){*/
			if ($ShowDiscountGP){
				...
				echo '<input class="number" maxlength="4" name="Discount_' . $OrderLine->LineNumber . '" required="required" size="5" title="' . _('Enter the discount percentage to apply to the price for this item') . '" type="hidden" value="' . locale_number_format(($OrderLine->DiscountPercent * 100),2) . '" />';
				echo '<input class="number" maxlength="40" name="GPPercent_' . $OrderLine->LineNumber . '" required="required" size="4" value="' . locale_number_format($OrderLine->GPPercent,2) . '" title="' . _('Enter a gross profit percentage to use as the basis to calculate the price to charge the customer for this line item') . '" type="hidden" />';
			}
            echo '<td><input type="text" name="LGroup_' . $OrderLine->LineNumber  . '" size="6" maxlength="6"  value="' . $OrderLine->LGroup . '" /></td>';
                echo '<td><input type="text" name="LSection_' . $OrderLine->LineNumber  . '" size="6" maxlength="6" value="' . $OrderLine->LSection . '" /></td>';
                echo '<td><input type="text" name="LCost_' . $OrderLine->LineNumber  . '" size="6" maxlength="6" value="' . $OrderLine->LCost . '" /></td>';
                echo '<td><input type="text" name="Source_' . $OrderLine->LineNumber  . '" size="6" maxlength="6" value="' . $OrderLine->Source . '" /></td>'; 
	...
	if (DB_num_rows($result)==0){
		...
		while ($myrow=DB_fetch_array($result)){
			if (isset($_POST['LSection']) AND $_POST['LSection']==$myrow['sectionid']){
				echo '<option selected="False" value="' . $myrow['sectionid'] . '">' . $myrow['sectionid'] . ' - ' . $myrow['sectionname'] . '</option>';
			} else {
