...
if (isset($_FILES['userfile']) and $_FILES['userfile']['name']) { //start file processing
	...
	while ( ($myrow = fgetcsv($FileHandle, 10000, ',')) !== FALSE ) {
		...
		if ($myrow[5] != 0) {
			$sql = "SELECT COUNT(groupid) FROM locationgroup WHERE groupid='" . $myrow[5] . "'";
			$result = DB_query($sql);
			$TestRow = DB_fetch_row($result);
			if ($TestRow[0] == 0) {
				$InputError = 1;
				prnMsg (_('Group ID') . ' "'. $myrow[5]. '" ' . _('does not exist'),'error');
			}
		}

		//Then check that the tag ref is either zero, or exists in the tags table
		if ($myrow[6] != 0) {
			$sql = "SELECT COUNT(sectionid) FROM locationsection WHERE sectionid='" . $myrow[6] . "'";
			$result = DB_query($sql);
			$TestRow = DB_fetch_row($result);
			if ($TestRow[0] == 0) {
				$InputError = 1;
				prnMsg (_('Section ref') . ' "'. $myrow[6]. '" ' . _('does not exist'),'error');
			}
		}

		//Then check that the tag ref is either zero, or exists in the tags table
		if ($myrow[7] != 0) {
			$sql = "SELECT COUNT(costid) FROM locationcost WHERE costid='" . $myrow[7] . "'";
			$result = DB_query($sql);
			$TestRow = DB_fetch_row($result);
			if ($TestRow[0] == 0) {
				$InputError = 1;
				prnMsg (_('Cost ref') . ' "'. $myrow[7]. '" ' . _('does not exist'),'error');
			}
		}

		//Then check that the tag ref is either zero, or exists in the tags table
		if ($myrow[8] != 0) {
			$sql = "SELECT COUNT(sourceref) FROM source WHERE sourceref='" . $myrow[8] . "'";
			$result = DB_query($sql);
			$TestRow = DB_fetch_row($result);
			if ($TestRow[0] == 0) {
				$InputError = 1;
				prnMsg (_('Source ref') . ' "'. $myrow[8]. '" ' . _('does not exist'),'error');
			}
		}
	...
	echo '<div class="page_help_text">' .
			...
			$FieldHeadings[0] . ', ' . $FieldHeadings[1] . ', ' . $FieldHeadings[2] . ', ' . $FieldHeadings[3] . ', ' . $FieldHeadings[4] . ', ' . $FieldHeadings[5] . ', ' . $FieldHeadings[6] . ', ' . $FieldHeadings[7] . ', ' . $FieldHeadings[8] . '<br />' .
